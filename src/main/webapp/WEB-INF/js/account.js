...
function option(obj) {
    var personalMessage = document.querySelector("#personalMessage").innerHTML
    var shouhuodizhi = document.querySelector("#SHDZ").innerHTML
    var uploadHeader = document.querySelector("#uploadHeader").innerHTML
    $(".list-group a").removeClass("active")
    $(obj).addClass("active");
    var op = obj.innerText;
    if (op.trim() == "个人资料") {
        $(".right").empty();
        $(".right").append(personalMessage)
        $(".msg #userName").val(userName)
        $("#email").text(email)
    }
    if (op.trim() == "收货地址") {
        $(".right").empty();
        $(".right").append(shouhuodizhi)
        getProvince()
...
function chooseImg(obj) {
    ...
    re.onload = function (re) {
        $('#chooseImg').attr("src", re.target.result);
        $('.card2 img').attr("src", re.target.result);
        $("#reChoose label").hover(function () {
            $("#reChoose label").css("cursor", "pointer")
        })
        $('.imgNow').html("预览头像");
        $("#chooseImg").fadeIn();
        $("#chooseImg").fadeIn("slow");
        $("#chooseImg").fadeIn(3000);
        $(".card1").css("border", "0")
        $("#reChoose").css("opacity", "100%")
        $(".sub input").removeClass("disabled")
        $(".sub input").mouseenter(function () {
            $(".sub input").css("background-color", "rgb(0,181,229)")
        })
        $(".sub input").mouseleave(function () {
            $(".sub input").css("background-color", "rgb(0,161,214)")
        })
...
function personMsg() {
    var userName=$(".msg #userName").val();
    var email=$("#email").text();
    var sign=$(".msg textarea").val();
    var gender=$(".msg input:radio:checked").val();
    var msg={"userName":userName,"email":email,"sign":sign,"gender":gender}
    $.ajax({
        url: "/accountMsg",
        type:"post",
        data:msg,
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            email = data.email;
            $(".msg #userName").val(data.userName)
            $("#email").text(data.email)
        }
    })
    $(".right").append(document.querySelector("#personalMessage").innerHTML)
})